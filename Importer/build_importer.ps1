...
$Spec = Join-Path $ImporterDir 'NFSMW Asset Importer.spec'
$Requirements = Join-Path $ImporterDir 'requirements.txt'
...
function Copy-ToExeSimple([string]$ExePath) {
    $Bundle = Get-ParentPath $ImporterDir
    if (-not $Bundle) { $Bundle = $ImporterDir }
...
if (-not (Test-Path -LiteralPath $Spec)) {
    throw "No esta el spec del importador en: $Spec"
}

$PythonExe = Find-VenvPython $ImporterDir
if (-not $PythonExe) {
...
if (-not $PythonExe) {
    ...
    if (-not $Launcher) {
        throw 'No hay Python. Instala Python 3.10 y vuelve a ejecutar Compilar importer.bat'
    }
    $VenvDir = Join-Path $ImporterDir '.venv'
...
if (-not (Test-Path -LiteralPath $PythonExe)) {
    throw 'No se encontro python.exe del entorno virtual.'
}

Write-Host "Python: $PythonExe"
Ensure-BuildDeps $PythonExe $Requirements
Set-Location -LiteralPath $ImporterDir

$Tests = Join-Path $ImporterDir 'tests\test_asset_importer.py'
if (Test-Path -LiteralPath $Tests) {
    & $PythonExe -m unittest tests.test_asset_importer -q
    if ($LASTEXITCODE -ne 0) { throw 'Fallaron las pruebas del importador.' }
}

& $PythonExe -m PyInstaller --noconfirm --clean $Spec
if ($LASTEXITCODE -ne 0) { throw 'Fallo la compilacion del NFSMW Asset Importer.' }

$Exe = Join-Path $ImporterDir 'dist\NFSMW Asset Importer.exe'
