...
title Compilar launcher
cd /d "%~dp0"
set "HERE=%CD%"
for %%P in ("%HERE%\..") do set "PARENT=%%~fP"
set "SIMPLE=%PARENT%\ExeSimple"
set "SPEC=%HERE%\Need For Speed Most Wanted Android.spec"
set "EXE=%HERE%\dist\Need For Speed Most Wanted Android.exe"
set "REQ=%HERE%\requirements.txt"
set "PYTHON="
set "ANYPY="

echo ============================================
echo  Compilar launcher
echo  Los EXE listos iran a ExeSimple
echo ============================================
echo.

if not exist "%SPEC%" (
  echo No esta el spec del launcher en: %SPEC%
  goto :fail
)
...
echo %PYTHON% | find /i ".venv\Scripts\python.exe" >nul
if not errorlevel 1 (
  for %%A in ("%PYTHON%") do set "SCRIPTS=%%~dpA"
  for %%A in ("!SCRIPTS!..\..") do set "NFSMW_PROJECT_ROOT=%%~fA"
)

echo.
"%PYTHON%" -m PyInstaller --noconfirm --clean "%SPEC%"
if errorlevel 1 (
  echo Fallo la compilacion del launcher.
  goto :fail
)
...
echo Copia facil: %SIMPLE%\Need For Speed Most Wanted Android.exe
echo.
echo Listo.
pause
exit /b 0
...
:CreateVenv
set "PYTHON="
set "VENVDIR=%PARENT%\.venv"
call :FindAnyPython
if not defined ANYPY (
  echo No hay Python. Instala Python 3.10 y vuelve a ejecutar Compilar launcher.bat
  goto :eof
)
